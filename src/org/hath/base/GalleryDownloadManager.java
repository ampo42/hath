...
public class GalleryDownloadManager implements Runnable {
	...
	private File hhdldir, downloadeddir, completeddir;
	...
	public GalleryDownloadManager(HentaiAtHomeClient client) {
		...
		try {
			...
			downloadeddir = FileTools.checkAndCreateDir(new File(Settings.getDownloadDir()));
			completeddir = FileTools.checkAndCreateDir(new File(Settings.getCompletedDir()));
				...
				if(! pendingGalleries.isEmpty()) {
					...
					if(doDownload) {
						...
						for(Gallery g : pendingGalleries) {
							...
							int galleryState = g.getState();
							
							if(galleryState != Gallery.STATE_PENDING) {
								completed.add(g);
							}
						}
						...
						for(Gallery g : completed) {
							...
							// move gallery to the completeddir, if necessary
							if(!completeddir.equals(downloadeddir)){
								File completedgallerydir = null;
								try {
									completedgallerydir = FileTools.checkAndCreateDir(new File(completeddir, g.getDir().getName()));
									if(FileTools.move(g.getDir(), completedgallerydir)){
										Out.info("Downloader: Successfully moved gallery "+g.getTitle()+" to "+completedgallerydir.getPath()+" .");
									}else{
										Out.warning("Downloader: Failed to move gallery "+g.getTitle()+" to "+completeddir.getPath()+" .");
									}
								} catch(java.io.IOException e) {
									Out.warning("Downloader: Failed to move gallery "+g.getTitle()+" to "+completeddir.getPath()+" .");
								}
						...
						if(! requestTokens.isEmpty()) {
							// request up to 10 tokens a minute from the server
							
							if(lastTokenRequest < System.currentTimeMillis() - 60000) {
