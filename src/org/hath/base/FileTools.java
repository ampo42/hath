...
public class FileTools {
	...
	/**
	 * <p>Move <code>fromFile</code> to <code>toFile</code>.
	 * <code>fromFile</code> can either be a file or a directory.</p>
	 * @param <code>fromFile</code> - The source File
	 * @param <code>toFile</code> - The destination File
	 * @return <code>true</code> if it successfully moved the File
	 */
	public static boolean move(File fromFile, File toFile) {
		if (!fromFile.exists()){
			Out.error("FileTools.move: File "+fromFile.getPath()+" does not exist.");
			return false;
		}
		
		// simply try to rename. Won't work if the file systems differ or
		// if the destination file already exists.
		if(fromFile.renameTo(toFile))
			return true;
		
		// here, either the destination file already exists or is in a 
		// different file system.
		if(fromFile.isDirectory()){
			if(!toFile.isDirectory() && !toFile.mkdirs()){
				Out.error("FileTools.move: Cannot create directory "+toFile.getPath()+" .");
				return false;
			}
			// move the content of the directory
			File[] content = fromFile.listFiles();
			for (File file: content){
				if(!move(file, new File(toFile,file.getName())))
					return false;
			}
		}else if(toFile.isFile()){
			// the destination file exists.
			Out.warning("FileTools.move: Destination file "+toFile.getPath()+" already exists and will not be overwritten." );
			return false;
		}else{
			// probably a different file system, we need to copy the file
			if(!copy(fromFile,toFile)) return false;
		}
		// if we arrive here, we can safely remove fromFile
		fromFile.delete();
